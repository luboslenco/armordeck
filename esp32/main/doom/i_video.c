...
// byte I_VideoBuffer[SCREENWIDTH*SCREENHEIGHT];
byte I_VideoBuffer[SCREENWIDTH*(SCREENHEIGHT + 28)];
...
void I_InitGraphics (void)
{
    ...
    // } else {
        //fb_scaling = 1;//s_Fb.xres / SCREENWIDTH;
...
void I_SetPalette (byte* palette)
{
    ...
    for (i=0; i<256; ++i ) {
        ...
        c = (col_t*)palette;
        colors[i] = GFX_RGB565(gammatable[usegamma][c->r],
                                gammatable[usegamma][c->g],
                                gammatable[usegamma][c->b]);
        palette += 3;
    }
