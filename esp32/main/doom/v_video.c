...
#define SCREENHEIGHT_FULL (SCREENHEIGHT + 28)
                ...
                int destx, int desty)
{
    ...
    if (srcx < 0
     ...
     || srcy < 0
     || srcy + height > SCREENHEIGHT_FULL
     || destx < 0
     || destx + width > SCREENWIDTH
     || desty < 0
     || desty + height > SCREENHEIGHT_FULL)
    {
    ...
    if (desty + height > SCREENHEIGHT_FULL)
        height = SCREENHEIGHT_FULL - desty;
...
void V_DrawPatch(int x, int y, patch_t *patch)
{
    ...
    if (x < 0
     ...
     || y < 0
     || y + SHORT(patch->height) > SCREENHEIGHT_FULL)
    {
    ...
    col = 0;
    ////
    int dx = (SCREENWIDTH << FRACBITS) / SCREENWIDTH;
    int dxi = (SCREENWIDTH << FRACBITS) / SCREENWIDTH;
    if (x < 0)
    {
    ...
    ////
    int dy = (SCREENHEIGHT_FULL << FRACBITS) / SCREENHEIGHT_FULL;
    ...
    for ( ; col<w ; x++, col++, desttop++)
    {
        ...
        while (column->topdelta != 0xff)
        {
            ...
            if (top + count > SCREENHEIGHT_FULL)
            {
                count = SCREENHEIGHT_FULL - top;
            }
            if (count < 1)
            {
                break;
...
void V_DrawBlock(int x, int y, int width, int height, byte *src)
{
    ...
    dest = dest_screen + (y >> 1) * SCREENWIDTH + x; ////
...
{
    memcpy(dest_screen, raw, SCREENWIDTH * SCREENHEIGHT_FULL);
}
