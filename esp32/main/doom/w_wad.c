...
void W_AddFile (char *filename)
{
  ...
  //   }
    ...
    // {
        ...
        /*#ifdef _WIN32
        if (file_wad == NULL) {
            FILE *fstream = fopen("./DOOM1.WAD", "rb");
        ...
        wadinfo_t *header_ptr = &file_wad[0];
        #else*/
        wadinfo_t *header_ptr = &doom1_wad[0];
        //#endif
        ...
        int first_lump_pos = LONG(header_ptr->infotableofs);
        //#ifdef _WIN32
        //filelumps = (filelump_t*)&file_wad[first_lump_pos];
        //#else
        filelumps = (filelump_t*)&doom1_wad[first_lump_pos];
        //#endif
        numlumps += LONG(header_ptr->numlumps);
        // newnumlumps += header_ptr->numlumps;
    // }
...
{
    //#ifdef _WIN32
    //return &file_wad[LONG(filelumps[lump].filepos)];
    //#else
    return &doom1_wad[LONG(filelumps[lump].filepos)];
    //#endif
}
...
char *W_LumpName(unsigned int lump)
{
...
void W_ReadLump(unsigned int lump, void *dest)
{
    ...
    void *ptr = &doom1_wad[LONG(filelump->filepos)];
...
void *W_CacheLumpNum(int lumpnum, int tag)
{
    ...
    //{
        ...
        // result = doom1_wad + lump->position;
    //}
